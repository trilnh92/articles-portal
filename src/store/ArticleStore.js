...
class ArticleStore {
    ...
    @computed get filteredArticles() {
        return this.categoryId > 0
            ? this.articles.filter(
                  article => article.categoryId === this.categoryId
              )
            : this.articles;
    }

    @action
    filteredFeaturedArticles = () =>
        toJS(this.articles.filter(article => article.isFeaturedArticle));
    ...
    getArticleById = articleId =>
        toJS(this.articles.find(article => article.id === articleId));
